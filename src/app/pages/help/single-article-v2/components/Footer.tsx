...
const features: FeatureItem[] = [
  { icon: 'tabler:copy', title: 'Over 1 million listings' },
  { icon: 'tabler:file-search', title: 'Personalized search' },
  { icon: 'tabler:cash-banknote-edit', title: 'Online car appraisal' },
  { icon: 'tabler:bulb', title: 'Non-stop innovation' },
]
...
const footerSections: FooterSection[] = [
  {
    ...
    links: [
      ...
      { label: 'Online car appraisal', href: '' },
    ],
  },
  {
    title: 'About',
    links: [
      { label: 'About Finder', href: '' },
  ...
  },
  {
    ...
    links: [
      ...
      { label: 'Add listings', href: '' },
    ],
  },
]

const socialLinks: SocialItem[] = [
  { icon: 'tabler:brand-instagram', label: 'Instagram' },
  { icon: 'tabler:brand-facebook-filled', label: 'Facebook' },
  { icon: 'tabler:brand-x', label: 'X (Twitter)' },
]

const Footer = () => {
  return (
    <footer className="bg-body-bg dark md:pt-6">
      <div className="container">
        <div className="grid md:grid-cols-4 grid-cols-2 gap-6 pt-12 pb-8 border-b border-default-300 dark:border-white/20">
          {features.map((feature, index) => (
            <div key={index} className="flex gap-2">
              <Icon icon={feature.icon} className="size-6 text-white/80" />
              <h6 className="text-default-700 font-semibold">{feature.title}</h6>
            </div>
          ))}
        </div>

        <div className="md:py-12 pt-0 grid lg:grid-cols-4 md:grid-cols-12 grid-cols-1 md:gap-6">
          {footerSections.map((section, index) => (
            <div key={index} className={`${index === 0 ? 'lg:col-span-1 md:col-span-3' : 'lg:col-span-1 md:col-span-2'} col-span-1`}>
              ...
              <div className="hs-accordion-group md:hidden block">
                <div className="hs-accordion border-b border-white/20 py-3">
                  <button className="hs-accordion-toggle inline-flex w-full items-center gap-x-3 text-start text-base font-semibold text-white" aria-expanded="false" type="button">
                    {section.title}
          ...
          <div className="lg:col-span-1 md:col-span-5 col-span-1 md:mt-0 mt-10 md:mb-0 mb-6">
            <span className="mb-2 block text-white font-semibold">Download our app</span>

            <p className="mb-6 text-sm text-white/70">Download Finder App and join the community of car enthusiasts.</p>

            <div>
              <div className="flex shrink-0 justify-center  gap-3 pt-xl-3 pe-xl-3 me-xxl-5">
                <Link className="flex items-center gap-2 py-3 px-4.5 rounded-xl transition-all duration-300 bg-default-300/60 hover:bg-default-300" href="" aria-label="Download on App Store">
                  <Icon icon="tabler:brand-apple-filled" className="text-white size-6" />
                  <svg className="shrink-0 text-white" xmlns="http://www.w3.org/2000/svg" width={77} height={24} viewBox="0 0 77 24" fill="currentColor">
                ...
                <Link className="flex items-center py-3 px-4.5 rounded-xl transition-all duration-300 bg-default-300/60 hover:bg-default-300" href="" aria-label="Download on App Store">
                  <svg className="shrink-0 ms-n1 me-2" xmlns="http://www.w3.org/2000/svg" width={24} height={24} viewBox="0 0 24 24" fill="none">
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 pt-4 pb-6 lg:py-6 md:justify-between">
          <p className="md:order-1 order-2 text-center text-sm text-white/60">
          ...
          <div className="md:order-2 order-1 flex">
            {socialLinks.map((social, index) => (
              <div key={index} className="group hs-tooltip flex flex-col items-center justify-center [--placement:auto]">
                <button type="button" className="hs-tooltip-toggle inline-flex size-10 items-center justify-center gap-2">
                  <Icon icon={social.icon} className="text-white" />

                  <span role="tooltip" className="hs-tooltip-content invisible absolute z-10 -mt-4 -ms-5 inline-block py-1 px-2 text-xs text-white/70 hs-tooltip-shown:visible hs-tooltip-shown:opacity-100">
                    {social.label}
                  </span>
                </button>
              </div>
